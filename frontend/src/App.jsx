      ...
      <div className="main-content">
        <div className={`dashboard-container ${activeTab === 'caisse' || activeTab === 'agenda' ? 'wide' : ''}`}>
              ...
              {role === 'gerant' && activeTab === 'accueil' && (
                <div className="admin-container" style={isMobile ? { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'var(--bg-app)', transition: 'background-color 0.3s ease', display: 'flex', flexDirection: 'column', padding: 'max(24px, env(safe-area-inset-top)) 16px 0 16px' } : {}}>
                  {/* EN-TÊTE FIXE */}
                  <div style={isMobile ? { flexShrink: 0, backgroundColor: 'var(--bg-app)', transition: 'background-color 0.3s ease', zIndex: 10, paddingBottom: '8px' } : {}}>
                                    ...
                                    <div className="grid-collaborateurs">
                                        {(employesListe || []).map((emp, index) => (
                                            <div key={emp.id_employe} onClick={() => handleSelectEmployeCaisse(emp.id_employe)}
                                                style={{ backgroundColor: COULEURS_EMPLOYES[index % COULEURS_EMPLOYES.length], color: '#111827', padding: '24px 12px', borderRadius: 'var(--radius-card)', fontSize: '16px', fontWeight: '600', textAlign: 'center', cursor: 'pointer', boxShadow: 'var(--shadow-sm)', transition: 'transform 0.15s ease, box-shadow 0.15s ease' }} onMouseOver={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = 'var(--shadow-md)'; }} onMouseOut={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = 'var(--shadow-sm)'; }}>
              ...
              {role === 'gerant' && activeTab === 'actions' && (
                <div className="admin-container" style={isMobile ? { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'var(--bg-app)', transition: 'background-color 0.3s ease', display: 'flex', flexDirection: 'column', padding: 'max(24px, env(safe-area-inset-top)) 16px 0 16px', zIndex: 10 } : { display: 'flex', flexDirection: 'column', minHeight: '100%' }}>
                  {/* EN-TÊTE FIXE */}
                  <div style={isMobile ? { flexShrink: 0, backgroundColor: 'var(--bg-app)', transition: 'background-color 0.3s ease', zIndex: 10, paddingBottom: '8px' } : {}}>
                      <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px'}}>
                          <div>
             ...
             {activeTab === 'agenda' && (
              ...
              {role === 'gerant' && activeTab === 'superadmin' && decodeToken(token)?.id_salon === 38 && (
                <div className="admin-container" style={isMobile ? { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'var(--bg-app)', transition: 'background-color 0.3s ease', display: 'flex', flexDirection: 'column', padding: 'max(24px, env(safe-area-inset-top)) 16px 0 16px', zIndex: 10 } : {}}>
                  <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px'}}>
